...
const EmployeeUpdate = ({ closeModal, type, User }: { closeModal?: () => void, type: string, User: any }) => {
    const { id } = useParams();
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (isEmptyField || validationErrors.status == true) {
            dispatch(setNotice({ name: 'editEmployee', msg: 'Please fill valid data in all mandotory fields', code: 1 }))
            ...
            if (type == 'Personalform') {
                ...
                DataToUpdate = {
                    ...
                    email: Inputdata.employment.email,
            ...
            if (type == 'Employmentform') {
                axios.post('http://localhost:5000/editUser', {
                    email: Inputdata.employment.email,
                    uid: User?.id
                }).then((userRecord) => {
            ...
            DataToUpdate && firestore.collection("users").doc(id).set(DataToUpdate, { merge: true })
                .then((res) => {
                    dispatch(setNotice({ name: 'editEmployee', msg: 'Employee details updated successfully!', code: 3, time: 3000 }));
                    block && block.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    setTimeout(() => {
                        closeModal?.();
                    }, 3000);
